...
class TollController extends GetxController {
  ...
  Future<void> fetchBillCountries() async {
    ...
    try {
      final response = await Get.find<NetworkService>().get(
        endpoint: "${ApiPath.getBillCountriesEndpoint}/toll",
      );
  ...
  Future<void> fetchPayBillServices() async {
    ...
    try {
      final response = await Get.find<NetworkService>().get(
        endpoint:
            "${ApiPath.getPayBillServicesEndpoint}/${countryController.text}/toll",
      );
      if (response.status == Status.completed) {
  ...
  bool validateAmountStep() {
    ...
    if (countryController.text.isEmpty) {
      ToastHelper().showErrorToast(localization!.tollCountryRequired);
      return false;
    }

    // Validate Service
    if (serviceController.text.isEmpty) {
      ToastHelper().showErrorToast(localization!.tollServiceRequired);
      return false;
    }

    // Validate Amount
    if (amountText.value.isEmpty && amountController.text.isEmpty) {
      ToastHelper().showErrorToast(localization!.tollAmountRequired);
      return false;
    }

    final amount = double.tryParse(amountController.text) ?? 0.0;
    if (amount <= 0) {
      ToastHelper().showErrorToast(localization!.tollAmountValid);
      return false;
    }
    ...
    for (var entry in dynamicFieldControllers.entries) {
      ...
      if (fieldData.text.trim().isEmpty) {
        ToastHelper().showErrorToast(
          localization!.tollDynamicFieldRequired(fieldName),
        );
        return false;
      }
    }

...
class DataBundleController extends GetxController {
  ...
  Future<void> fetchBillCountries() async {
    ...
    try {
      final response = await Get.find<NetworkService>().get(
        endpoint: "${ApiPath.getBillCountriesEndpoint}/data-bundle",
      );
  ...
  Future<void> fetchPayBillServices() async {
    ...
    try {
      final response = await Get.find<NetworkService>().get(
        endpoint:
            "${ApiPath.getPayBillServicesEndpoint}/${countryController.text}/data-bundle",
      );
      if (response.status == Status.completed) {
  ...
  bool validateAmountStep() {
    ...
    if (countryController.text.isEmpty) {
      ToastHelper().showErrorToast(localization!.dataBundleCountryRequired);
      return false;
    }

    // Validate Service
    if (serviceController.text.isEmpty) {
      ToastHelper().showErrorToast(localization!.dataBundleServiceRequired);
      return false;
    }

    // Validate Amount
    if (amountText.value.isEmpty && amountController.text.isEmpty) {
      ToastHelper().showErrorToast(localization!.dataBundleAmountRequired);
      return false;
    }

    final amount = double.tryParse(amountController.text) ?? 0.0;
    if (amount <= 0) {
      ToastHelper().showErrorToast(localization!.dataBundleAmountValid);
      return false;
    }
    ...
    for (var entry in dynamicFieldControllers.entries) {
      ...
      if (fieldData.text.trim().isEmpty) {
        ToastHelper().showErrorToast(
          localization!.dataBundleDynamicFieldRequired(fieldName),
        );
        return false;
      }
    }

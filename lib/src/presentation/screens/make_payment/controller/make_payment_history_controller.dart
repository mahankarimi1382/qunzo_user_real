...
class MakePaymentHistoryController extends GetxController {
  // Global Variable
  ...
  Future<void> fetchTransactions() async {
    try {
      ...
      final response = await Get.find<NetworkService>().get(
        endpoint:
            '${ApiPath.transactionsEndpoint}?type=payment&page=${currentPage.value}&per_page=15',
      );
  ...
  Future<void> loadMoreTransactions() async {
    ...
    try {
      ...
      final endpoint =
          '${ApiPath.transactionsEndpoint}?type=payment&${queryParams.join('&')}';
